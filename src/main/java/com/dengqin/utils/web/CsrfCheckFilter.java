...
import com.dengqin.utils.base.CookieUtil;
import com.dengqin.utils.base.EncryptUtil;
import com.dengqin.utils.base.Util;
import com.dengqin.utils.exception.CsrfTokenInvalidException;
 ...
 * <filter> <filter-name>CsrfCheckFilter</filter-name>
 * <filter-class>com.dengqin.utils.web.CsrfCheckFilter</filter-class>
 * <!-- 忽略接口 --> <init-param> <param-name>ignoreUrl</param-name>
 * <param-value>/webservice/abc.do,/webservice/to.do</param-value> </init-param>
 * <!-- 是否仅写日志:true|false --> <init-param> <param-name>logOnly</param-name>
 * <param-value>true</param-value> </init-param> </filter> <filter-mapping>
 * <filter-name>CsrfCheckFilter</filter-name> <url-pattern>*.do</url-pattern>
 * </filter-mapping>
 *
 * @author lixinchao
 */
public class CsrfCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (!isIgnore) {
			...
			if (!isPass) {
				log.warn("check csrf fail ,token:" + token + ",username:" + username + ",proxyIp:" + proxyIp
	...
	private boolean checkToken(String username, String token) {
		...
		if (!sha1.startsWith(prefix)) {
			log.warn("prefix:" + prefix + " username:" + username + " time:" + time + " sha1:" + sha1 + " token:"
					+ token);
			return false;
		}
		return true;
